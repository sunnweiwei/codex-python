...
from .types import CodexConfig, CodexResult, PromptRequest
...
class MemoryBackgroundTask:
    def __init__(self, *, kind: str, target: Any):
        ...
        self._target = target
        self._thread = threading.Thread(target=self._run, name=f"codex-{kind}", daemon=True)
        self._thread.start()
...
class MemoryStateStore:
    ...
    def open_codex_home(cls, codex_home: Path | str) -> "MemoryStateStore":
        return cls(Path(codex_home) / "state" / "memory.sqlite3")
...
def memory_rollout_candidates(codex_home: Path | str, limit: int = 5_000) -> list[Path]:
...
def backfill_memory_threads_from_rollouts(
    ...
    *,
    ...
    updated = 0
    for path in memory_rollout_candidates(codex_home, limit=limit):
        try:
...
def reset_memory_workspace_baseline(root: Path | str) -> None:
    ...
    if git_path.is_dir() and not git_path.is_symlink():
        ...
        git_path.unlink()
    _git(root_path, "init")
    _git(root_path, "config", "user.name", "Codex")
...
def build_memory_consolidation_config(
    *,
    ...
    config = base_config or CodexConfig()
