...
class CodexSession:
    """Python-native implementation of the public `codex exec` core loop."""
    ...
    def _local_compact_request(
        ...
        return PromptRequest(
            ...
            instructions=build_base_instructions(
                ...
                codex_home=compact_config.resolved_codex_home(),
    ...
    def _dispatch_tool_calls(self, tool_calls: list[dict[str, Any]]) -> Iterator[CodexEvent]:
        parallel_enabled = self.config.resolved_parallel_tool_calls()
        pending_parallel: list[dict[str, Any]] = []

        def flush_parallel() -> Iterator[CodexEvent]:
    ...
    def _inspect_pending_input(self, item: dict[str, Any]) -> Iterator[CodexEvent]:
    ...
    def _record_hook_additional_contexts(self, outcome: _HookOutcome) -> Iterator[CodexEvent]:
    ...
    def _run_hook(self, event_name: str, **payload: Any) -> Iterator[CodexEvent]:
...
def _config_with_memory_state_store(config: CodexConfig) -> CodexConfig:
    if not _should_run_memory_startup(config) or config.memory_state_store is not None:
        return config
    try:
        state_store = MemoryStateStore.open_codex_home(config.resolved_codex_home())
    except Exception:
        return config
    return replace(config, memory_state_store=state_store)


def _should_run_memory_startup(config: CodexConfig) -> bool:
    return (
...
def _memory_rate_limit_snapshot(config: CodexConfig) -> Any | None:
...
def _model_stream_retry_delay_seconds(config: CodexConfig, retry: int, error: Any) -> float:
...
def _provider_supports_remote_compaction(config: CodexConfig) -> bool:
