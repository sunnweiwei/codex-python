...
from .types import ApprovalPolicy, CodexConfig, NetworkAccess, SandboxMode
...
def build_initial_context_items(config: CodexConfig, *, cwd: Path | None = None) -> list[dict[str, Any]]:
    ...
    if config.memory_tool_enabled and config.use_memories:
        memory_instructions = build_memory_tool_developer_instructions(config.resolved_codex_home())
...
def build_memory_tool_developer_instructions(codex_home: Path, summary_token_limit: int = 5_000) -> str | None:
    base_path = codex_home / "memories"
