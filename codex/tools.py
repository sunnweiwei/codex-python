...
UPSTREAM_MODELS_JSON = Path(__file__).resolve().parent / "upstream" / "openai-codex" / "codex-rs" / "models-manager" / "models.json"
UPSTREAM_SEATBELT_BASE_POLICY = Path(__file__).resolve().parent / "upstream" / "openai-codex" / "codex-rs" / "sandboxing" / "src" / "seatbelt_base_policy.sbpl"
...
class ToolRuntime:
    ...
    def apply_patch(self, arguments: Any) -> ToolResult:
        ...
        if _looks_like_codex_freeform_patch(patch):
            return self._apply_codex_patch(patch, cwd=cwd)
    ...
    def _apply_codex_patch(self, patch: str, *, cwd: Path | None = None) -> ToolResult:
        cwd = cwd or self.cwd
        try:
            ...
            affected = self._apply_codex_patch_changes_with_approval(
                patch,
                changes,
                cwd=cwd,
            ...
            return ToolResult(False, str(exc), {"strategy": "codex apply_patch", "returncode": 1})
    ...
    def _apply_codex_patch_changes_with_approval(
        self,
        patch: str,
        ...
        *,
        ...
        try:
            return _apply_codex_patch_changes(cwd, changes, writable_roots=writable_roots)
        except _PatchApplicationError:
            raise
        except ValueError as exc:
            if self.config.sandbox != "workspace-write" or not _is_workspace_escape_error(exc):
                raise
            files = _codex_patch_touched_paths(changes)
            approval_error = self._apply_patch_approval_error(
                patch=patch,
                files=files,
                reason=str(exc),
            ...
            if approval_error is not None:
                raise
            affected = _apply_codex_patch_changes(cwd, changes, writable_roots=None)
...
def request_user_input_spec(config: CodexConfig) -> dict[str, Any]:
...
def view_image_spec(config: CodexConfig | None = None) -> dict[str, Any]:
    properties = {
...
def web_search_spec(config: CodexConfig) -> dict[str, Any]:
    spec: dict[str, Any] = {
        "type": "web_search",
...
def _supports_image_input(config: CodexConfig) -> bool:
...
def _can_request_original_image_detail(config: CodexConfig) -> bool:
...
_CODEX_PATCH_HEREDOC_START_LINES = {"<<EOF", "<<'EOF'", '<<"EOF"'}


def _looks_like_codex_freeform_patch(patch: str) -> bool:
    lines = patch.strip().splitlines()
    if not lines:
        return False
    if lines[0].strip() == "*** Begin Patch":
        return True
    return _has_codex_patch_heredoc_wrapper(lines)


def _has_codex_patch_heredoc_wrapper(lines: list[str]) -> bool:
    return (
        len(lines) >= 4
        and lines[0] in _CODEX_PATCH_HEREDOC_START_LINES
...
def _parse_codex_patch(patch: str) -> list[dict[str, Any]]:
    lines = patch.strip().splitlines()
    if _has_codex_patch_heredoc_wrapper(lines):
        lines = lines[1:-1]
...
def _codex_patch_touched_paths(changes: list[dict[str, Any]]) -> list[str]:
    paths: list[str] = []
...
def _apply_codex_patch_changes(
    cwd: Path,
    changes: list[dict[str, Any]],
    *,
    writable_roots: list[Path] | None = None,
) -> _PatchAffected:
    _verify_codex_patch_changes(cwd, changes, writable_roots=writable_roots)
...
def _verify_codex_patch_changes(
    cwd: Path,
    changes: list[dict[str, Any]],
    *,
    writable_roots: list[Path] | None,
