...
from .types import CodexConfig, CodexEvent, new_thread_id, new_turn_id
...
class CodexState:
    ...
    def rollout_path(self) -> Path:
        if self._rollout_path is None:
            ...
            sessions = self.config.resolved_codex_home() / "sessions"
    ...
    def _persist_event(self, event: CodexEvent) -> None:
    ...
    def _rollout_records_for_event(self, event: CodexEvent) -> list[dict[str, Any]]:
        records = []
...
def _estimate_prompt_visible_response_item_token_count(item: dict[str, Any], config: CodexConfig) -> int:
...
def _sandbox_policy(config: CodexConfig) -> dict[str, Any]:
...
def _file_system_sandbox_policy(config: CodexConfig) -> dict[str, Any]:
    if config.sandbox == "danger-full-access":
...
def _truncation_policy(config: CodexConfig) -> dict[str, Any] | None:
...
def _turn_started_payload(state: CodexState) -> dict[str, Any]:
    return {
        "type": "task_started",
...
def _turn_completed_payload(state: CodexState, event: CodexEvent) -> dict[str, Any]:
...
def _turn_aborted_payload(state: CodexState, event: CodexEvent) -> dict[str, Any]:
    completed_at = time.time()
    duration_ms = None
    if state._turn_started_at is not None:
        duration_ms = int((completed_at - state._turn_started_at) * 1000)
...
def _token_count_payload(event: CodexEvent) -> dict[str, Any]:
    return {
        "type": "token_count",
...
def _stream_error_payload(event: CodexEvent) -> dict[str, Any]:
    return _drop_none(
        {
            "type": "stream_error",
...
def _hook_started_payload(event: CodexEvent) -> dict[str, Any]:
    return {
        "type": "hook_started",
...
def _hook_completed_payload(event: CodexEvent) -> dict[str, Any]:
    return {
        "type": "hook_completed",
        "hook_event_name": str(event.payload.get("name") or ""),
...
def _write_stdin_event_payload(state: CodexState, event: CodexEvent) -> list[dict[str, Any]] | None:
    metadata = event.payload.get("metadata")
    if not isinstance(metadata, dict):
        return None
    call_id = str(event.payload.get("call_id") or "")
...
def _tool_workdir(state: CodexState, args: dict[str, Any]) -> Path:
...
def _tool_path(state: CodexState, value: str) -> Path:
...
def _track_apply_patch_change(state: CodexState, change: dict[str, Any]) -> bool:
...
def _turn_diff_unified_diff(state: CodexState) -> str | None:
...
def _turn_diff_rename_pairs(state: CodexState) -> dict[str, str]:
    pairs: dict[str, str] = {}
