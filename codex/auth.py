...
def resolve_auth_home(codex_home: Path | str | None = None) -> Path:
    if codex_home is not None:
        return Path(codex_home).expanduser().resolve()
    explicit = os.environ.get("CODEX_AUTH_HOME")
    if explicit:
        return Path(explicit).expanduser().resolve()
    official_env = os.environ.get("CODEX_HOME")
    if official_env:
        return Path(official_env).expanduser().resolve()
    official_home = Path("~/.codex").expanduser()
    if (official_home / "auth.json").exists():
        return official_home.resolve()
    return official_home.resolve()


def auth_json_path(codex_home: Path | str | None = None) -> Path:
    return resolve_auth_home(codex_home) / "auth.json"
...
) -> CodexAuthSnapshot | None:
    requested_mode = normalize_auth_mode(mode)
    path = auth_json_path(codex_home)
    try:
...
def refresh_chatgpt_auth(snapshot: CodexAuthSnapshot, *, timeout: float = 60) -> CodexAuthSnapshot:
    ...
    request = urllib.request.Request(
        os.environ.get(REFRESH_TOKEN_URL_OVERRIDE_ENV_VAR, REFRESH_TOKEN_URL),
        data=body,
        headers={
            "Content-Type": "application/json",
            "Accept": "application/json",
            "originator": ORIGINATOR,
            "User-Agent": f"{ORIGINATOR}/python-codex",
        },
        method="POST",
    )
    try:
...
def login_with_api_key(api_key: str, codex_home: Path | str | None = None) -> Path:
...
def complete_device_code_login(
    ...
    *,
    ...
    return save_chatgpt_tokens(codex_home=codex_home, **tokens)
...
def auth_status(codex_home: Path | str | None = None) -> dict[str, Any]:
    path = auth_json_path(codex_home)
    try:
        raw = json.loads(path.read_text(encoding="utf-8"))
    except FileNotFoundError:
...
def fetch_chatgpt_rate_limits(
    ...
    *,
    ...
    """Fetch Codex ChatGPT usage windows using the official backend shape."""

    snapshot = load_auth_snapshot(codex_home, mode="chatgpt")
    if snapshot is None:
...
def parse_chatgpt_rate_limit_payload(payload: dict[str, Any]) -> list[ChatGPTRateLimitSnapshot]:
    """Map `/api/codex/usage` or `/backend-api/wham/usage` JSON to Codex snapshots."""
...
def chatgpt_codex_base_url(configured: str | None = None) -> str:
    base = (
        os.environ.get("CODEX_CHATGPT_CODEX_BASE_URL")
...
def chatgpt_backend_base_url(configured: str | None = None) -> str:
    base = (
        os.environ.get("CODEX_CHATGPT_BACKEND_BASE_URL")
...
def _refresh_snapshot_if_needed(snapshot: CodexAuthSnapshot, *, timeout: float) -> CodexAuthSnapshot:
...
def _chatgpt_backend_headers(snapshot: CodexAuthSnapshot) -> dict[str, str]:
...
def _api_key_snapshot(path: Path, raw: dict[str, Any]) -> CodexAuthSnapshot | None:
    api_key = raw.get("OPENAI_API_KEY")
    if not isinstance(api_key, str) or not api_key:
        api_key = os.environ.get("OPENAI_API_KEY")
    if not api_key:
        return None
    return CodexAuthSnapshot(mode="apikey", source_path=path, api_key=api_key, raw=raw)


def _chatgpt_snapshot(path: Path, raw: dict[str, Any], auth_mode: str) -> CodexAuthSnapshot | None:
...
class _LoginCallbackHandler(http.server.BaseHTTPRequestHandler):
    def do_GET(self) -> None:  # noqa: N802 - BaseHTTPRequestHandler API.
        ...
        try:
            ...
            result_path = save_chatgpt_tokens(codex_home=context["codex_home"], **tokens)
            context["result_path"] = result_path
            self._send_html(
                200,
                "<h1>Successfully logged in</h1><p>You can close this browser window and return to Codex.</p>",
            )
