...
class GoalStore:
    ...
    Python core keeps the same thread-keyed protocol state in a codex-home JSON
    file so the runtime stays self-contained.
    """

    def __init__(self, codex_home: Path | str):
        self.path = Path(codex_home).expanduser().resolve() / "thread_goals.json"
...
class GoalRuntime:
    def __init__(self, config: CodexConfig, state: Any):
        self.config = config
        self.state = state
        self.store = GoalStore(config.resolved_codex_home())
        self._lock = threading.RLock()
